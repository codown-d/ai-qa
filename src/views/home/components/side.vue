<template>
  <div class="side_con">
    <div class="nav">
      <div
        :class="['nav_item', currentNav === item.id ? 'active_item' : '']"
    ...
    <el-scrollbar class="list">
      <el-menu
        ...
        @select="(index: string) => selectItem({ id: index })"
      >
        ...
        <el-sub-menu index="knowledge">
          <template #title>
            ...
            <span @click.stop="selectItem({ id: 'knowledge/lib' })" style="width: 100%;"
              >知识库</span
            >
          </template>
          <el-menu-item
            v-for="item in knowledgeStore.knowledgeList"
            :index="`knowledge/${item.name}`"
            :key="item.name"
          >
            <div
              class="cricle"
            ...
            <div>{{ item.name }}</div>
            <img
              src="@/assets/images/pencil_edit.svg"
              @click.stop="
  ...
  getInfo().then((res) => {
    if (res.data.code === 1000) {
      knowledgeStore.userInfo = merge(res.data.data, {
        avatar_url: `${import.meta.env.VITE_BASE_URL}${
